...
function handleSessionEvent(session, event) {
  ...
  try {
    if (event.type === 'assistant/chunk') {
      ...
      console.log('[L2D-SPK-HOST] event=assistant/chunk chunkType=' + (c && c.type)
        + ' text=' + JSON.stringify(c && typeof c.text === 'string' ? c.text.slice(0, 40) : undefined)
        + ' blockType=' + (c && c.block && c.block.type)
        + ' openTools=' + openTools.size)
    } else {
      console.log('[L2D-SPK-HOST] event=' + event.type + (event.type === 'tool/call' ? ' name=' + (event.data?.name ?? event.data?.message?.name) : ''))
    }
  } catch {}
