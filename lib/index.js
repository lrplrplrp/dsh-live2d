...
// host 端代码版本标记（临时调试）：随 state 端点/SSE 一起下发，
// 用于确认运行中的 host 是否已加载最新 index.js。确认后删除。
const HOST_BUILD = 'host-spk3'

/** 临时调试日志（host 控制台），确认后删除。 */
function L2DDBG_HOST(...a) {
  try { console.log('[L2D-SPK-HOST]', ...a) } catch {}
}
...
function setState(state) {
  if (currentState !== state) {
    ...
    // 临时调试：确认 host 侧状态流转（排查 SPEAKING）
    L2DDBG_HOST('setState ->', state)
...
function broadcastState(state) {
  const payload = `data: ${JSON.stringify({ state, updatedAt: stateUpdatedAt, hostBuild: HOST_BUILD })}\n\n`
  for (const res of stateClients) {
    try { res.write(payload) } catch {}
  }
}

// 临时调试：把 host 侧的判断依据经同一条 SSE 推给浏览器，方便一次性抓完整日志。
// 确认问题后连同 L2DDBG 一起删除。
function broadcastDebug(note) {
  const payload = `data: ${JSON.stringify({ dbg: note, t: stateUpdatedAt })}\n\n`
  for (const res of stateClients) {
    try { res.write(payload) } catch {}
  }
}
...
function handleSessionEvent(session, event) {
  ...
  // 临时调试：记录每个会话事件（assistant/chunk 额外打出 chunk 类型），
  // 并推送到浏览器控制台（浏览器侧日志更便于一次性抓取）。
  try {
    if (event.type === 'assistant/chunk') {
      const c = event.data?.chunk
      broadcastDebug('evt chunk type=' + (c && c.type)
        + ' text=' + JSON.stringify(c && typeof c.text === 'string' ? c.text.slice(0, 30) : undefined)
        + ' block=' + (c && c.block && c.block.type)
        + ' openTools=' + openTools.size)
    } else {
      broadcastDebug('evt ' + event.type + (event.type === 'tool/call' ? ' name=' + (event.data?.name ?? event.data?.message?.name) : ''))
    }
  } catch {}
  ...
  switch (event.type) {
    ...
    case 'turn/end': {
      ...
      // 本轮是否输出过正文（对话）。是的话，本轮结束时先把「输出对话」状态多保留一会儿
      // 再切到完成/错误 —— 因为不少链路下正文不是流式的：assistant/message 一到，
      // turn/end 紧随其后，SPEAKING 只存在几毫秒，看起来像「结束时才闪一下」。
      const hadDialogue = speakingLocked || sawDialogueThisTurn
      ...
      if (kind === 'completed') {
        if (hadDialogue) {
          // 按正文长度估算“阅读/输出”时长后，再进入完成状态（有上下限）
          const holdMs = clampDialogueDwell(lastDialogueChars)
          L2DDBG_HOST('turn/end completed: hold SPEAKING ' + holdMs + 'ms (chars=' + lastDialogueChars + ')')
          broadcastDebug('turn/end: hold SUCCESS ' + holdMs + 'ms (dialogueChars=' + lastDialogueChars + ')')
          scheduleTerminalState(DSHState.SUCCESS, holdMs, 2500)
        } else {
          broadcastDebug('turn/end: no dialogue -> SUCCESS immediately')
      ...
      httpCtx.effect(
        () => httpCtx.webServer.register({
          ...
          handler: (req, res) => {
            ...
            jsonResponse(res, 200, { state: currentState, updatedAt: stateUpdatedAt, hostBuild: HOST_BUILD })
          },
        }),
      ...
      httpCtx.effect(
        () => httpCtx.webServer.register({
          ...
          handler: (req, res) => {
            ...
            // 立即发送当前状态，避免新连接错过最新值
            res.write(`data: ${JSON.stringify({ state: currentState, updatedAt: stateUpdatedAt, hostBuild: HOST_BUILD })}\n\n`)
