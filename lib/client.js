...
window.__ModuleLoader__.load({ id: 'dsh-live2d', factory: (require) => {
  ...
  const ANIMATION_STATES =['IDLE', 'THINKING', 'THINK_END', 'WORKING', 'SUCCESS', 'ERROR']
  ...
  const DEFAULT_MODEL_LIST = {
    models: [{
      ...
      animations: {
        ...
        THINKING: { motion: { group: 'Anima', index: 2 } },
        THINK_END: { motion: { group: 'Anima', index: 1 } },
        WORKING: { motion: { group: 'Anima', index: 0 } },
  ...
  class Live2DEngine {
    ...
    // 强制切换：先停止当前正在播放的 motion，再播放新的，避免动画叠加
    _stopCurrentMotion() {
      if (!this.model) return
      try {
          ...
          setCfg(function(prev) {
            ...
            var merged = (data.models || []).map(function(h) {
              ...
              return {
                ...
                // 动画映射：优先保留用户已有配置；否则使用 host 从内置 model_list.json 合并的默认映射（安装即可用）
                animations: ex.animations || h.animations || {},
              }
            })
    ...
    return h('div', { style: { padding: '6px 10px 20px', fontSize: 13, lineHeight: 1.6 } },
      ...
      currentModel ? h('div', { style: Object.assign({}, cardStyle, { marginTop: 16 }) },
        ...
        ANIMATION_STATES.map(function(stateName) {
          ...
          function applyMotion(nextGroup, nextIndex) {
            var next = {}
            if (nextGroup) {
            ...
            if (!nextGroup && !next.motion && !selExpr) next = {}
          ...
          return h('div', { key: stateName, style: Object.assign({}, rowStyle, { padding: '6px 0', borderBottom: '1px solid rgba(127,127,127,0.15)' }) },
            ...
            h('div', { style: { display: 'flex', gap: 8, alignItems: 'center', marginLeft: 8 } },
              ...
              h('select', {
                ...
                disabled: !selGroup,
                onChange: function(e) {
                  applyMotion(selGroup, e.target.value)
                },
              ...
              h('select', {
                ...
                onChange: function(e) {
                  ...
                  if (val) {
                    updateAnimationWithForceSwitch(activeModel, stateName, selGroup ? { motion: { group: selGroup }, expression: val } : { expression: val })
                  } else {
                    updateAnimationWithForceSwitch(activeModel, stateName, selGroup ? { motion: { group: selGroup } } : {})
                  }
                },
