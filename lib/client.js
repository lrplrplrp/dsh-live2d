...
window.__ModuleLoader__.load({ id: '@lrplrplrp/dsh-live2d', factory: (require) => {
  ...
  // ── 调试标记：版本号 + 模块加载即打印，用于确认浏览器跑的是哪一份 bundle ──
  const DBG_VERSION = 'v20250912k'
  const DBG = (...a) => { try { console.log('[L2D-DBG][' + DBG_VERSION + ']', ...a) } catch (e) {} }
  DBG('MODULE LOADED', { href: location.href, rev: window.__DSH_BOOT__ && window.__DSH_BOOT__.rev })
  ...
  // 首开欢迎播完后的停留时长（毫秒）：期间挂起时间映射，让用户看清/听清欢迎再交回
  ...
  function unlockAudioOnce() {
    ...
    if ((target || pendingTimeMapping) && _engineInstance) {
      DBG('unlock replay @', new Date().toLocaleTimeString(), 'target=' + target, 'pendingTimeMapping=' + pendingTimeMapping)
      try {
      ...
      } catch (e) { DBG('unlock replay error', e) }
    } else {
      DBG('unlock fired', { target, pendingTimeMapping, hasEngine: !!_engineInstance })
    }
  }
  ...
  class Live2DEngine {
    ...
    async     init(containerEl) {
      ...
      this.app.ticker.add(this._eyeFollowTick = () => {
        ...
        // 首开页面欢迎待确认/播放期：守卫由 _welcomePending（playAnimation 的
        // IDLE 分支）与 playWelcomeFirstOpen 内部的 checkTimeMappings 控制，
        // 这里照常调度时间映射即可——_welcomePending 期间不会抢触发。
    ...
    async playAnimation(stateName) {
      ...
      if (stateName === 'IDLE') {
        // 首开会话“仅播欢迎”：完全不触发时间映射
        if (this._firstOpenSuppressing) { DBG('playAnimation IDLE suppressed (first-open, welcome only)'); return }
        // 首开页面欢迎待确认期：暂不抢触发时间映射，等欢迎状态决定
        if (this._welcomePending) { DBG('playAnimation IDLE blocked by _welcomePending'); return }
        const mappings = this.getTimeMappings()
        const idx = this.findActiveTimeMapping(mappings)
        DBG('playAnimation IDLE', 'timeMappingIdx=' + idx, 'audioUnlocked=' + _audioUnlocked, 'activeTimeMapping=' + this._activeTimeMapping)
        if (idx >= 0) {
          // 音频未解锁（页面加载期、早于任意用户手势）：时间映射语音会被自动播放策略
          // 静音，故整体推迟，先不播；记录命中下标，待首次手势解锁后由 checkTimeMappings 补播
          // （动作+语音一同出声）。已解锁则直接播，并记录命中下标。
          if (!_audioUnlocked) {
            DBG('playAnimation IDLE: audio locked, DEFER time mapping', 'idx=' + idx)
    ...
    async playAutoMotion(group, index, reason) {
      ...
      if (_audioUnlocked) {
        // 已解锁：直接播
        DBG('playAutoMotion PLAY', 'reason=' + (reason || '?'), 'group=' + group, 'index=' + index)
        await this.triggerMotion(group, index, reason)
        return true
      }
      // 未解锁：记录目标，待解锁后补播（仅保留最近一个；欢迎优先于时间映射）
      DBG('playAutoMotion DEFER', 'reason=' + (reason || '?'), 'group=' + group, 'index=' + index, 'deferredAuto=' + _deferredAuto)
    ...
    async playWelcomeFirstOpen() {
      ...
      DBG('playWelcomeFirstOpen', 'hasWelcomAnim=' + !!anim, 'audioUnlocked=' + _audioUnlocked, 'welcomePending=' + this._welcomePending, 'welcomeGroup=' + (anim && anim.motion ? anim.motion.group : '<none>') + ':' + (anim && anim.motion ? anim.motion.index : '<none>'))
    ...
    async _playWelcomeNow() {
      ...
      DBG('welcome played (first-open: welcome only, time mapping stays suppressed)')
    }
    ...
    async checkTimeMappings() {
      if (!this.model) return
      // 首开会话：本次只播欢迎，完全抑制时间映射（刷新后 _firstOpenSuppressing 已为 false，
      // 时间映射照常）。欢迎播完（停留期结束）后由 _playWelcomeNow 清除该标记。
      if (this._firstOpenSuppressing) {
        DBG('checkTimeMappings suppressed (first-open, welcome only)')
        this._activeTimeMapping = -1
        this._timeMappingDeferred = false
        return
      }
      // 首开页面欢迎优先期：等待标记确认及欢迎动作播完前，不抢触发时间映射
      //（等待期由 _welcomePending 守护；播放期由 _welcomePlayed 守护；结束后
      // 由 playWelcomeFirstOpen 主动调用本函数接管）。
      if (this._welcomePending || this._welcomePlayed) { DBG('checkTimeMappings early-return', { welcomePending: this._welcomePending, welcomePlayed: this._welcomePlayed }); return }
      ...
      this._timeMappingDeferred = false
      if (idx >= 0) {
        // 命中时间段：播放映射动画。但音频尚未解锁（早于任意用户手势）时，浏览器自动播放
        // 策略会静音，故整体推迟——记录命中下标，待首次手势解锁后由 unlockAudioOnce 补播
        // （动作+语音一同出声）。已推迟的同段不再重复记录，避免 ticker 每帧刷日志。
        if (!_audioUnlocked) {
          if (this._timeMappingDeferred && this._activeTimeMapping === idx) return
          DBG('checkTimeMappings DEFER time mapping (audio locked)', 'idx=' + idx)
          this._activeTimeMapping = idx
          this._timeMappingDeferred = true
          return
        }
        DBG('checkTimeMappings PLAY time mapping', 'idx=' + idx, 'audioUnlocked=' + _audioUnlocked)
    ...
    async triggerMotion(group, index, reason) {
      ...
      try {
        const mm = this.model.internalModel.motionManager
        const def = (mm && mm.definitions && mm.definitions[group] && mm.definitions[group][index]) || null
        soundFile = def && (def.Sound || def.sound) || null
      } catch (e) {}
      DBG('triggerMotion', 'reason=' + (reason || '?'), 'group=' + group, 'index=' + index, 'audioUnlocked=' + _audioUnlocked, 'sound=' + soundFile)
      // 强制切换掉正在播放的动画（含循环中的）
      this._stopCurrentMotion()
      try {
        await this.model.motion(group, index)
      } catch (e) { DBG('triggerMotion error', e) }
    }
    ...
    useEffect(() => {
      ...
      engine.init(containerRef.current).then((ok) => {
        if (ok) {
          ...
          DBG('init welcome-decision', 'isFirstOpen=' + isFirstOpen, 'welcomedFlag=' + (() => { try { return window.sessionStorage.getItem('dsh-live2d-welcomed') } catch (e) { return '<err>' } })())
          if (isFirstOpen) {
            ...
            DBG('init first-open suppress time mapping?', hasWelcome)
        ...
        h('label', { style: { display: 'flex', alignItems: 'center', gap: 8, marginTop: 6, cursor: 'pointer' } },
          h('input', {
            ...
            onChange: function(e) {
              ...
              setCfg(function(prev) {
                var next = Object.assign({}, prev, { hitArea: enabled })
                saveConfig(next)
                return next
              })
            },
