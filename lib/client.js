...
window.__ModuleLoader__.load({ id: '@lrplrplrp/dsh-live2d', factory: (require) => {
  ...
  const L2DDBG = (...a) => { try { console.log('[L2D-SPK][' + DBG_VERSION + ']', ...a) } catch (e) {} }
  ...
  function Live2DWidget() {
    ...
    useEffect(() => {
      ...
      if (!engine || !engine.ready) { L2DDBG('state-effect SKIP (engine not ready)', 'dshState=' + dshState); return }

      const prevState = lastStateRef.current
      const newState = dshState
      lastStateRef.current = newState

      L2DDBG('state-effect', 'prev=' + prevState, 'new=' + newState)
      ...
      const run = async () => {
        if (prevState === DSHState.THINKING && newState !== DSHState.THINKING) {
          L2DDBG('state-effect: play THINK_END (transition)', 'for=' + newState)
          await engine.playAnimation('THINK_END')
        }
        // 过渡期间状态可能已再次变化，丢弃过期的一次
        if (lastStateRef.current !== newState) { L2DDBG('state-effect: DROP stale', 'new=' + newState, 'current=' + lastStateRef.current); return }
        L2DDBG('state-effect: play state', 'state=' + newState)
