...
window.__ModuleLoader__.load({ id: '@lrplrplrp/dsh-live2d', factory: (require) => {
  ...
  function unlockAudioOnce() {
    ...
    PERF('unlockAudioOnce: first user gesture -> unlock')
    _audioUnlocked = true
    ...
    if ((target || pendingTimeMapping) && _engineInstance) {
      try {
        if (target === 'welcome') {
          ...
          PERF('unlockAudioOnce: replay WELCOME (drop pending time mapping=' + pendingTimeMapping + ')')
          ...
          PERF('unlockAudioOnce: replay TIME MAPPING')
  ...
  function installAudioUnlock() {
    PERF('installAudioUnlock: listeners installed')
    const handler = () => {
      unlockAudioOnce()
  ...
  class Live2DEngine {
    ...
    async     init(containerEl) {
      ...
      PERF('engine.init start (waiting libs)')
      const ok = await ensureLibsLoaded()
      if (!ok) return false
      PERF('engine.init libs ready')
      ...
      // 加载模型
      PERF('model load start: ' + (modelEntry && modelEntry.name))
      await this.loadModel(activeIdx, 0)
      PERF('model load done')
    ...
    async playAutoMotion(group, index, reason) {
      ...
      PERF('playAutoMotion: defer as ' + _deferredAuto + ' (reason=' + reason + ')')
      return false
    }
    ...
    async playWelcomeFirstOpen() {
      PERF('playWelcomeFirstOpen: enter (audioUnlocked=' + _audioUnlocked + ', deferredAuto=' + _deferredAuto + ')')
      const modelList = getConfig()
      const modelEntry = modelList.models[this.currentModelIndex]
      ...
      if (!anim || _audioUnlocked) {
        PERF('playWelcomeFirstOpen: play now (hasAnim=' + !!anim + ', audioUnlocked=' + _audioUnlocked + ')')
        // 无有效欢迎动画 / 音频已解锁：立即播放（欢迎优先于加载期的时间映射）
        await this._playWelcomeNow()
        return
      }
      PERF('playWelcomeFirstOpen: DEFER until user gesture')
    ...
    async _playWelcomeNow() {
      ...
      try {
        ...
        PERF('_playWelcomeNow: welcome motion started')
      } catch {}
  ...
  function Live2DWidget() {
    ...
    useEffect(() => {
      ...
      PERF('Live2DWidget mounted (overlay slot rendered)')
      injectCSS()
      ...
      engine.init(containerRef.current).then((ok) => {
        if (ok) {
          ...
          PERF('widget: resync check, curState=' + cur)
  ...
  function apply(ctx) {
    PERF('client apply() called (plugin activated)')
    injectCSS()
