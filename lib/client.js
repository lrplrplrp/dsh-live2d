      ...
      const bootstrap = async () => {
        try {
          ...
          if (res.ok) {
            ...
            L2DDBG('poll state recv', 'state=' + data.state, 'hostBuild=' + data.hostBuild)
      ...
      try {
        ...
        es.onmessage = (ev) => {
          try {
            ...
            // 临时调试：host 侧的事件依据（排查完成后删除）
            if (data.dbg) { L2DDBG('HOST ' + data.dbg); return }
            // 确认运行中的 host 是否已加载最新 index.js
            if (data.hostBuild) L2DDBG('HOST_BUILD=' + data.hostBuild)
            if (data.state) feed.push(data.state)
          } catch {}
        }
        es.onerror = () => {
