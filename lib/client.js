...
window.__ModuleLoader__.load({ id: 'dsh-live2d', factory: (require) => {
  ...
  class Live2DEngine {
    ...
    async playAnimation(stateName) {
      ...
      // 循环播放：直到切换其他状态前，持续循环当前动作
      const loop = anim.loop === true

      if (anim.motion) {
        // 支持整组动画：未指定 index 时随机播放组内一个 motion
        if (typeof anim.motion.index === 'number') {
          await this.triggerMotion(anim.motion.group, anim.motion.index, loop)
        } else {
          await this.triggerMotionGroup(anim.motion.group, loop)
        }
      }
      if (anim.expression) {
    ...
    async triggerMotionGroup(group, loop) {
      ...
      try {
        ...
        await this.triggerMotion(group, index, loop)
      } catch {}
    }

    async triggerMotion(group, index, loop) {
      if (!this.model) return
      // 强制切换掉正在播放的动画（含循环中的）
      this._stopCurrentMotion()
      try {
        // pixi-live2d-display 的 model.motion 支持 { loop } 选项持续循环播放
        await this.model.motion(group, index, 3, loop ? { loop: true } : undefined)
      } catch {}
    }
